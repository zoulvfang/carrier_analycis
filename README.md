...
（1）针对订单量大且高运费订单数最多的州如SP、MG，PR增设区域配送仓，区域仓的位置参考异常高运费订单最多的TOP10路线，SP可以在本州再建立一个区域仓，同时分别在到货州RJ与MG各建立自己的区域仓，PR与MG可考虑在到货州SP分别建立自己的区域仓，以解决跨州远距离高运费难题。通过测算，若把20%的跨州运输转同州运输（运费平均降低73.7%），则总运费降幅约为14.7%（20%x73.7%）；
